...
describe('audit-C15 no duplicate autoPauseAdjustEnabled key', () => {
  ...
  test('the surviving declaration uses pickBool with default true', () => {
    const code = stripComments(modRaw);
    // Should be exactly: autoPauseAdjustEnabled: pickBool(o, b, 'autoPauseAdjustEnabled', true),
    const matches = code.match(/autoPauseAdjustEnabled\s*:\s*pickBool\(\s*o\s*,\s*b\s*,\s*['"]autoPauseAdjustEnabled['"]\s*,\s*true\s*\)/);
    expect(matches).not.toBeNull();
    expect(matches.length).toBe(1);
  });

  test('no stray duplicate field declarations exist for any autoPauseAdjust* key', () => {
    const code = stripComments(modRaw);
    // Walk every line and ensure no autoPauseAdjust* field appears twice in the same object literal
